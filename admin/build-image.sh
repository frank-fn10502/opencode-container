...
usage() {
  cat <<'USAGE'
Usage: admin/build-image.sh [--dockerfile FILE] [--build-arg KEY=VALUE]... [--skip-vm]
...
localhost/opencode-dev-yuta:<opencode-version>-env.<revision>, save it under
.docker_imgs/, then build the matching opencode-vm image.
...
Options:
  ...
  --skip-vm
              Build only the base opencode-dev image.
...
if [[ "${build_vm}" -eq 1 ]]; then
  printf '\nBuilding matching opencode-vm image...\n'
  bash "${SCRIPT_DIR}/build-vm-image.sh" --vm-revision "${VM_REVISION}"
fi
